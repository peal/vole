...
impl<E: Edge> std::ops::BitXor<&Perm> for &DigraphBase<E> {
    type Output = DigraphBase<E>;

    fn bitxor(self, perm: &Perm) -> Self::Output {
...
mod tests {
    ...
    fn cycle_perm_graph() {
        ...
        let p = Perm::from_vec(vec![1, 2, 0]);
        let c2 = Perm::from_vec(vec![1, 0]);
        let e = (&d) ^ (&p);
        let f = (&d) ^ (&c2);
        let g = (&f) ^ (&c2);
        assert_eq!(d, e);
