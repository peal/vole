...
VoleTestCanonical := function(maxpnt, grp, obj, VoleFunc, action)
    ...
    p := Random(grp);
    newobj := action(obj, p);
    ret := VoleFind.CanonicalPerm(grp, Flat([VoleFunc(obj)]) : conf := rec(points := maxpnt));
    if not(ret in grp) then
        return StringFormatted("A -Not in group! {} {} {}", grp, obj, ret);
    fi;
    newret := VoleFind.CanonicalPerm(grp, Flat([VoleFunc(newobj)]) : conf := rec(points := maxpnt));
...
QuickChecker := function(p, c)
    local ret1, ret2;
    ret1 := VoleFind.Group(c : conf := rec(points := p));
